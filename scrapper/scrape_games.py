...
def extract_pbp_features(pbp_file: Path, boxscore_file: Path, game_id: str) -> pd.DataFrame:
    ...
    def parse_player_from_play(play_text):
        ...
        if pd.isna(play_text):
            return None
        match = re.search(r'([A-Z][a-zA-Z]+ [A-Z][a-zA-Z]+(?: Jr\.| III| IV)?)', str(play_text))
    ...
    for _, play_row in df_pbp.iterrows():
        ...
        clock_str = str(play_row.get('clck', ''))
        minutes_left = None
        try:
            m, s = clock_str.split(':')
            minutes_left = int(m) + int(s) / 60.0
        except:
            try:
                minutes_left = float(clock_str)
            except:
                minutes_left = None
        home_score = play_row.get('hmScr')
        away_score = play_row.get('awyScr')
        try:
        ...
        is_clutch = (period >= 4 and minutes_left is not None and minutes_left <= 5)
        
        # Track substitutions
        if any(phrase in txt_lower for phrase in ['enters the game', 'checks in', 'substitutes in']) and player in txt:
            stats['substitutions'] += 1
            if stats['first_sub_time'] is None and pd.notna(game_time):
                stats['first_sub_time'] = game_time
        
        # Count touch involvement
        if any(word in main_clause for word in ['makes', 'misses', 'turnover', 'free throw', 'foul']) or 'assist' in txt_lower:
    ...
    normalize_name = lambda s: re.sub(r'[^a-z]', '', str(s).lower())
    normalized_player_stats = {normalize_name(k): v for k, v in player_stats_pbp.items()}
    features = []
    for _, row in df_bs.iterrows():
        ...
        pbp_stats = player_stats_pbp.get(player_name, {}) or normalized_player_stats.get(normalize_name(player_name), {})
